...
final class MeetingWorkflowTests: XCTestCase {
    ...
    @MainActor
    func testReportsProcessingStatusBeforeEachModelStep() async throws {
        let meeting = Meeting(
            id: "2026-07-04-1400-tgb-sit",
            title: "TGB SIT 進度會議",
            recordedAt: ISO8601DateFormatter().date(from: "2026-07-04T14:00:00+08:00")!,
        ...
        var reportedStates: [MeetingProcessingState] = []

        _ = try await workflow.buildTranscript(
            for: meeting,
        ...
        XCTAssertEqual(reportedStates, [.transcribing, .diarizing])
    }
}
